...
async function scrapeShopee(url) {
  try {
    ...
    const extractCanonicalUrl = (html) => {
      const canonicalMatch = html.match(/<link\s+data-rh="true"\s+rel="canonical"\s+href="([^"]*)"/);
      if (canonicalMatch) {
    ...
    const scriptMatch = html.match(/<script\s+type="text\/mfe-initial-data"[^>]*>([\s\S]+?)<\/script>/);
    
    if (!scriptMatch) {
      throw new Error('Could not find <script type="text/mfe-initial-data"> in the HTML');
    }
    ...
    const extractShippingCost = (initialState) => {
      const rwVariation = initialState.RW_VARIATION_SELECTION?.data?.itemLevel?.product_shipping;
      if (!rwVariation) return '';

      const preSelectedChannel = rwVariation.pre_selected_shipping_channel;
      if (!preSelectedChannel || !preSelectedChannel.price_before_discount) return '';
    ...
    const extractDescription = (initialState) => {
      const rwVariation = initialState.RW_VARIATION_SELECTION?.data?.itemLevel?.item;
    ...
    const productData = {
      ...
      weight_g: findAttributeValue(itemData.attributes, 'น้ำหนักของสินค้า') || '',
      ...
      image_url: itemData.image ? `https://down-th.img.susercontent.com/${itemData.image}` : '',
